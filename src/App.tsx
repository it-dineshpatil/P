...
export default function App() {
  ...
  const [partnerName, setPartnerName] = useState('Pranali');
  ...
  const updatePartnerName = async () => {
    ...
    try {
      const res = await fetch('/api/settings', {
        ...
        body: JSON.stringify({ key: 'partner_name', value: tempName }),
      });
      if (res.ok) {
      ...
      console.error('Failed to update partner name:', err);
    }
  };
  ...
  const handleChat = async (e: React.FormEvent) => {
    ...
    try {
      const model = genAI.models.generateContent({
        ...
        config: {
          systemInstruction: `You are ${partnerName}, your user's sweet, caring, and super supportive personal partner/best friend. 
            ...
            >
              <div className="p-6 border-b border-rose-100/50 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-2xl font-bold tracking-tight text-slate-800">{partnerName}</h1>
                </div>
                <button 
                  onClick={() => setIsSidebarOpen(false)}
                  ...
                  {isEditingName ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onBlur={updatePartnerName}
                        onKeyDown={(e) => e.key === 'Enter' && updatePartnerName()}
                        autoFocus
                        className="bg-white/50 border border-rose-200 rounded-lg px-2 py-0.5 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-rose-400 w-32"
                      />
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 group cursor-pointer" onClick={() => { setTempName(partnerName); setIsEditingName(true); }}>
                      <h2 className="font-bold text-slate-800 text-lg">{partnerName}</h2>
          ...
          <div className="max-w-3xl mx-auto relative">
            ...
            <div className="mt-4 md:mt-6 flex flex-col items-center gap-2">
              <p className="text-[10px] text-rose-400 uppercase tracking-[0.3em] font-black opacity-60">
                {partnerName} • Your Personal AI Partner
              </p>
              <motion.a 
